...
namespace e_systemApplicationAPI.Controllers
{
    ...
    public class ApplicationsController : ControllerBase
    {
        ...
        {
            var existingApplication = await _context.Applications.FirstOrDefaultAsync(x => x.Id == id);
            if (existingApplication != null)
            {
                existingApplication.Status = "Cancel";
        ...
        [HttpPut]
        public async Task<IActionResult> RejectApplication([FromBody] Application[] applications)
        {
            foreach (var item in applications)
            {
                var existingApplication = await _context.Applications.FirstOrDefaultAsync(x => x.Id == item.Id);
                if (existingApplication != null)
                {
                    ...
                    existingApplication.Status = item.Status;
                    await _context.SaveChangesAsync();
                    await _emailRepository.sendRejectEmail(existingApplication);
                }
            }
            return Ok(applications);
        }
